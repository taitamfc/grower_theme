...
<div class="<?php echo esc_attr($classes); ?>">
    ...
    <div class="wrap-portfolios-post row row-wrap <?php echo esc_attr($settings['columns']); ?> <?php echo esc_attr($show_filter_class); ?>">
        ...
        <?php $i= 1; while ( $query->have_posts() ) : $query->the_post();                                      
            ...
            ?>              
			<div class="item img-box  <?php echo esc_attr( $termsString ); ?>">				<?php 					if ( has_post_thumbnail() ) { 						echo sprintf('<img src="%s" class="img-tf-%s" alt="image">', \Elementor\Group_Control_Image_Size::get_attachment_image_src( get_post_thumbnail_id(), 'thumbnail', $settings ), $i);					}                                           				?>				<div class="tf-wrap-content-portfolio tf-style-portfolio">					<div class="tf-content-portfolio">						<ul class="tf-cat-portfolio">							<li><a href="<?php echo get_the_permalink(); ?>">Farm House </a></li>						</ul>						<div class="tf-title-portfolio title-32"><a href="<?php echo get_the_permalink(); ?>"><?php echo get_the_title(); ?></a> </div>						<a class="tf-readmore-portfolio" href="<?php echo get_the_permalink(); ?>">  </a>					</div>				</div>			</div>                 
        <?php $i++; endwhile; ?>
        <?php if ( $settings['carousel'] == 'yes' ): ?>
            </div>
        <?php endif; ?>
