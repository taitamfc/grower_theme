		...
		public function render_content() {
			...
			<div class="themesflat-options-control-field">
				<?php foreach ( $choices as $_value => $params ):  ?>
					...
					<label>
						<input type="radio" value="<?php themesflat_esc_attr( $_value ) ?>" data-target="<?php themesflat_esc_attr( $name ) ?>" name="<?php themesflat_esc_attr( $name ) ?>" <?php $this->link();  checked( $this->value(), $_value ) ?> />
