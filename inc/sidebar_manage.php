...
class sidebar_manager {
	...
	public static function add_sidebar() {
		...
		if ( ! $name || $name == 'null' ) {
			die( "alert('".esc_html__( 'Please input sidebar name and try again.', 'grower' )."')" );
		}
		$id = sidebar_manager::name_to_class( $name );
		if ( isset( $sidebars[ $id ] ) ) {
			die( "alert('".esc_html__( 'Sidebar already exists, please use a different name.', 'grower' )."')" );
		}
	...
	public static function admin_menu() {
		add_theme_page( esc_html__( 'Sidebars Manager', 'grower' ), esc_html__( 'Sidebars Manager', 'grower' ), 'manage_options', 'multiple_sidebars', array( 'sidebar_manager', 'admin_page' ) );
	}
	...
	public static function admin_page() {
		...
		<script>
			function remove_sidebar_name(elem,name){
				answer = confirm("<?php echo esc_html__( 'Are you sure you want to remove', 'grower' ); ?> " + name + "?\n<?php echo esc_html__( 'This will remove any widgets you have assigned to this sidebar.', 'grower' ); ?>");
				if(answer){
			...
			function add_sidebar_name(){
				var sidebar_name = prompt("<?php echo esc_html__( 'Sidebar Name:', 'grower' ); ?>","");
				add_sidebar(sidebar_name);
			}
		</script>
		<div class="wrap">
			<h2><?php echo esc_html__( 'Sidebar Generator', 'grower' ); ?></h2>
			<p>
				<?php echo esc_html__( 'The sidebar name is for your use only. It will not be visible to any of your visitors. A CSS class is assigned to each of your sidebar, use this styling to customize the sidebars.', 'grower' ); ?>
			</p>
			<br />
			<div class="add_sidebar">
				<a href="<?php echo esc_js( 'javascript:void(0)' ); ?>" onclick="return add_sidebar_name()" title="<?php echo esc_html__( 'Add a sidebar', 'grower' ); ?>" class="button-primary"><?php echo esc_html__( '+ Add Sidebar', 'grower' ); ?></a>
			</div>
			<br />
			<table class="widefat page" id="themesflat_table">
				<tr>
					<th><?php echo esc_html__( 'Name', 'grower' ); ?></th>
					<th><?php echo esc_html__( 'CSS class', 'grower' ); ?></th>
					<th><?php echo esc_html__( 'Remove', 'grower' ); ?></th>
				</tr>
				<?php
				$sidebars = sidebar_manager::get_sidebars();
				if ( is_array( $sidebars ) && ! empty( $sidebars ) ) {
					...
					foreach ( $sidebars as $sidebar ) {
						...
						<tr class="<?php echo esc_attr( $alt ); ?>">
							...
							<td><a href="<?php echo esc_js( 'javascript:void(0)' ); ?>" onclick="return remove_sidebar_name(this,'<?php echo esc_attr( $sidebar ); ?>');" title="<?php echo esc_html__( 'Remove this sidebar', 'grower' ); ?>"><?php echo esc_html__( 'remove', 'grower' ); ?></a></td>
						</tr>
						<?php
						$cnt++;
					}
					...
					<tr id="themesflat_noexist">
						<td colspan="3"><?php echo esc_html__( 'No Sidebars defined', 'grower' ); ?></td>
					</tr>
					<?php
				}
				?>
	...
	public static function edit_form() {
		...
		<div id='themesflat-sortables' class='meta-box-sortables'>
			<div id="themesflat_box" class="postbox " >
				<div class="handlediv" title="Click to toggle"><br /></div><h3 class='hndle'><span><?php echo esc_html__( 'Sidebars', 'grower' ); ?></span></h3>
				<div class="inside">
					<div class="themesflat_container">
						<input name="themesflat_edit" type="hidden" value="themesflat_edit" />

						<p><?php echo esc_html__( 'Please select the sidebar you would like to display.', 'grower' ); ?> <strong><?php echo esc_html__( 'Note:', 'grower' ); ?></strong> <?php echo esc_html__( 'You must first create the sidebar under', 'grower' ); ?> <strong><?php echo esc_html__( 'Appearance > Sidebars', 'grower' ); ?></strong>.
						</p>
						<ul>
							<?php
